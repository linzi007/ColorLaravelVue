...
class CreateOrderGoodsPaymentsTable extends Migration 
{
	public function up()
	{
		Schema::create('order_goods_payments', function(Blueprint $table) {
            $table->increments('id');
            $table->integer('quehuo_number', 5)->nullable()->default(0)->comment('缺货数量');
            $table->integer('jushou_number', 5)->nullable()->default(0)->comment('拒收数量');
            $table->integer('shifa_number', 5)->nullable()->default(0)->comment('实发数量');
